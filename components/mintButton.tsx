...
const mintClick = async (
    ...
    let OZONETHANKS = true;
    if (!process.env.NEXT_PUBLIC_OZONETHANKS) {
      OZONETHANKS = false;
      console.log("The Creator does not want to Contribute to Ozone labs 😒");
    }
    ...
    try {
          ...
          tx = tx.prepend(
            transferSol(umi, {
              destination: publicKey(
                "CndyCmrkVMV3QP4mh1C5819ndQdqTGszsPs5YHDcm2nq"
              ),
              amount: sol(Number(0.00803)),
            })
          );
        }
      ...
      const sendPromises = signedTransactions.map((tx, index) => {
        return umi.rpc
          ...
          .then((signature) => {
            console.log(
              `Transaction ${index + 1} resolved with signature: ${signature}`
            );
